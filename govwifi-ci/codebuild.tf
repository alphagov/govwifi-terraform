resource "aws_codebuild_project" "govwifi_codebuild_push_to_ecr" {
  for_each = toset(var.app_names)
  name          = "govwifi-codebuild-${each.key}-push-to-ecr"
...
resource "aws_codebuild_webhook" "govwifi_app_webhook" {
  ...
  for_each = toset(var.app_names)
  project_name = aws_codebuild_project.govwifi_codebuild_project_push_to_ecr[each.key].name
  build_type   = "BUILD"
  filter_group {
    filter {
      type    = "EVENT"
...
###
### Codebuild Convert image format
