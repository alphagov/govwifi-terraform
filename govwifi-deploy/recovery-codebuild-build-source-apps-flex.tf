...
**/
resource "aws_codebuild_project" "govwifi_codebuild_get_source_apps_recovery" {
  for_each      = toset(var.deployed_app_names)
  name          = "recovery-${each.key}-source"
...
resource "aws_codebuild_webhook" "govwifi_app_webhook_recovery" {
  for_each     = toset(var.deployed_app_names)
  project_name = aws_codebuild_project.govwifi_codebuild_get_source_apps_recovery[each.key].name
