resource "aws_codebuild_project" "govwifi_codebuild_project_push_image_to_ecr_recovery" {
  for_each      = toset(var.deployed_app_names)
  name          = "${each.key}-push-image-to-ecr-recovery"
...
resource "aws_codebuild_webhook" "govwifi_app_webhook_recovery" {
  for_each = toset(var.deployed_app_names)

  project_name = aws_codebuild_project.govwifi_codebuild_project_push_image_to_ecr_recovery[each.key].name
