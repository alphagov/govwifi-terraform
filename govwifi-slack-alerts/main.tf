  ...
  template_body = <<-STACK
  {
    "Resources": {
      ...
      "Properties" : {
          ...
          "SnsTopicArns" : [ "${var.critical_notifications_topic_arn}","${var.capacity_notifications_topic_arn}","${var.route53_critical_notifications_topic_arn}", "${var.smoke_tests_alerts_topic_arn}" ]
        }
      }
    }
  }
